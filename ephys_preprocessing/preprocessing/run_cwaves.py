    ...
    for probe_id in probe_ids:
        ...
        for kilosort_folder in kilosort_folders:
            ...
            if sys.platform.startswith('win'):
                C_waves_fullpath = 'C_waves'
                shell = True
            elif sys.platform.startswith('linux'):
                C_waves_fullpath = config['cwaves_path']
                C_waves_fullpath = C_waves_fullpath.replace('\\', '/') + "/runit.sh"
                shell = False
            else:
                raise NotImplementedError('OS not recognised')

            command = [C_waves_fullpath,
            ...
            subprocess.run(command, shell=shell, cwd=config['cwaves_path'])
